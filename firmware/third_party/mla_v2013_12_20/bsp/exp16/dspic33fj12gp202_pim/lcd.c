...
#define LCD_SendData(data) {  LATC |= 0x00FF; 	   LATC |= data; LATBbits.LATB7  = 1;Nop();Nop();Nop(); LATBbits.LATB7  = 0;  LCD_Wait(LCD_F_INSTR); }
#define LCD_SendCommand(command, delay) {  int holder = (LATC&0xF00)>>8;LATC |= command|(holder<<8); LATBbits.LATB7  = 1;Nop();Nop();Nop();LATBbits.LATB7  = 0; LATC |= 0x00FF; LCD_Wait(delay); }
