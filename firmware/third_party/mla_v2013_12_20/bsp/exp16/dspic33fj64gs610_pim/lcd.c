...
bool LCD_Initialize ( void )
{
    ...
    TRISD &= 0xF0C0 ;        //set RD0-3 & RD8-11 as data output & RD4-5 as control line outputs
    TRISBbits.TRISB15 = 0 ;  // set RB15 as control line output
    ADPCFGbits.PCFG15 = 1 ;  // set RB15 as digital pin
...
static void LCD_SendData ( char data )
{
    ...
    loNibble = data & 0x0f ;
    hiNibble = data & 0xf0 ;
    packData = ( ( (uint16_t) hiNibble << 4 ) ) | ( loNibble ) ;
    ...
    // make sure the control signals are maintained
    packData = packData | ( LATD & 0x0030 ) ;
    LATD = packData ;
...
static void LCD_SendCommand ( char command , unsigned int delay )
{
    ...
    loNibble = command & 0x0f ;
    hiNibble = command & 0xf0 ;
    packData = ( ( (uint16_t) hiNibble << 4 ) ) | ( loNibble ) ;
